...
def get_walls_edges(wall, wall_lines):
	"""return all line boundaries of walls, append to wall_lines"""
	wall_lines.append([wall[0], wall[1]])
	wall_lines.append([wall[1], wall[3]])
	wall_lines.append([wall[3], wall[2]])
	wall_lines.append([wall[2], wall[0]])


def get_object_bbox(object_loc, object_radius):
	"""return 4 line segments of a bbox of an square object"""
	pt1 = [(object_loc[0] - object_radius), (object_loc[1] - object_radius)]
	pt2 = [(object_loc[0] + object_radius), (object_loc[1] - object_radius)]
	pt3 = [(object_loc[0] - object_radius), (object_loc[1] + object_radius)]
	pt4 = [(object_loc[0] + object_radius), (object_loc[1] + object_radius)]
	line1 = [pt1, pt2]
	line2 = [pt1, pt3]
	line3 = [pt2, pt4]
	line4 = [pt3, pt4]
...
def line_intersection(a, b, c, d):
	"""return intersection point of two lines if any, otherwise None"""
...
def if_intersect(ray, line):
	"""determine if two line intersects"""
	# NOTE: ray can be any line segments, line can ONLY be horizontal/vertical
...
class GzbGw(gym.Env):
	...
	def __init__(self, dfa, actions=3, random_loc=True, monitor=False, update_img=False, horizon=2000, rays=5):
		"""
		...
		"Observation Space"
		# self.RAY_COUNT * 4 for ray distance and ray detections
		# 3 for a_x, a_y, a_theta
		# 10 for 1-hot DFA
		self.observation_space = spaces.Box(low=0, high=1, shape=(self.RAY_COUNT * 4 + 3 + 10,), dtype=np.float32)
		...
		self.accepted_dfa_rew = 1 / self.HORIZON
		self.MAXDIST = 500 * math.sqrt(2)
		self.TURNPUNISH = 0.01 / self.HORIZON
		"Walls & Edges"
		self.EDGE_LEN = 500
		self.WALL_LEN = 100
		self.WIDTH = 15
		...
		# horizontal walls
		(x, y) = (50, 160)
		wall = [(x, y), (x + self.WALL_LEN, y), (x, y + self.WIDTH), (x + self.WALL_LEN, y + self.WIDTH)]
		self.walls.append(wall)
		get_walls_edges(wall, self.wall_lines)
		(x, y) = (350, 170)
		wall = [(x, y), (x + self.WALL_LEN, y), (x, y + self.WIDTH), (x + self.WALL_LEN, y + self.WIDTH)]
		self.walls.append(wall)
		get_walls_edges(wall, self.wall_lines)
		...
		# vertical walls
		(x, y) = (245, 60)
		wall = [(x, y), (x + self.WIDTH, y), (x, y + self.WALL_LEN), (x + self.WIDTH, y + self.WALL_LEN)]
		self.walls.append(wall)
		get_walls_edges(wall, self.wall_lines)
		(x, y) = (130, 360)
		wall = [(x, y), (x + self.WIDTH, y), (x, y + self.WALL_LEN), (x + self.WIDTH, y + self.WALL_LEN)]
	...
	def step(self, action):
		...
		if self.time_step == self.HORIZON:
			self.done = True
			print("horizon {} reached".format(self.HORIZON))
		self.success = False
		...
		if not collision_with_walls(self.wall_near_range, new_p1_location) and new_p1_location[1] < 600 / 2:
		...
		if not collision_with_walls(self.wall_near_range, new_p2_location) and new_p2_location[1] > 600 / 2:
		...
		"Lidar Rays"
		# first, use new_agent_location to check detection with walls
		lidar_rays = get_lidar_rays(new_agent_location, self.agent_angle, self.RAY_LEN, self.RAY_COUNT)
		actual_rays = deepcopy(lidar_rays)
		self.detections = [[0, 0, 1]] * self.RAY_COUNT  # walls=[0, 0, 1], o1=[0, 1, 0], 02=[1, 0, 0]
		lidar_distance = [self.RAY_LEN] * self.RAY_COUNT
		for i, ray in enumerate(lidar_rays):  # first, deal with walls
			intersection, min_dist = ray_intersect_shape(lidar_rays[i], self.wall_lines)
			if intersection is None:
				continue
			if min_dist < lidar_distance[i]:
				lidar_rays[i] = ray[0], intersection
				actual_rays[i] = ray[0], intersection
				lidar_distance[i] = min_dist
			if min_dist < self.A_RADIUS * 1.1:
				w_collide = True
		# if NOT collide with walls
		if not w_collide:
			# update previous lidar with walls information & continue to deal with objects
			self.lidar_rays = lidar_rays
			self.actual_rays = actual_rays
			self.lidar_distance = lidar_distance
			for i, ray in enumerate(self.lidar_rays):  # then, deal with objects
		...
		# collide with walls
		else:  # redo calculations with the original agent location
			...
			for i, ray in enumerate(self.lidar_rays):  # first, deal with walls
		...
		if reach_goal(new_agent_location, self.position2, 5 * self.A_RADIUS):
			b_reach = True
		"HARD CODING HEURISTICS HERE"
		d1 = math.dist(new_agent_location, self.position1)
		d2 = math.dist(new_agent_location, self.position2)
		if self.dfa_state == '1':  # need to go to a -> award d1
			dist_curr = d1
		elif self.dfa_state == '3':  # need to go to b -> award d2
			dist_curr = d2
		else:
			dist_curr = self.dist_prev
		if self.dist_prev != -1 and action == 2:
			r = (self.dist_prev - dist_curr) / (self.STEP_SIZE * 1.25 * self.HORIZON * 10)
		else:
			r = 0
		self.reward += r
		self.dist_prev = dist_curr
		"Update DFA stuff"
		...
		if original_dfa_state != new_dfa_state:  # state change
			...
			if self.dfa_state in self.dfa.intermediate:  # state change to intermediate
				...
				self.reward += self.accepted_dfa_rew / 3
			if self.dfa_state == '3':
				self.dist_prev = d2  # reset dist_prev
			elif self.dfa_state == '4':
				self.dist_prev = -1
		"Rewards according to DFA"
		if self.dfa_state in self.dfa.acc:  # specification satisfied, goal reached
			self.reward += self.accepted_dfa_rew
		...
		self.display_img()
		"Observations"
		ax, ay = self.agent_location[0] / 600, self.agent_location[1] / 600
		theta = self.agent_angle / (math.pi * 2)
		dfa = np.zeros(10, dtype=np.float32)
		dfa[int(self.dfa_state) - 1] = 1
		lidar_distance = np.array(self.lidar_distance, dtype=np.float32).flatten() / self.MAXDIST
		detections = np.array(self.detections, dtype=np.float32).flatten()
		a_locations = np.array([ax, ay, theta], dtype=np.float32).flatten()
		self.observation = np.concatenate((dfa, a_locations, lidar_distance, detections), axis=None)
		"Info"
		info = {
	...
	def reset(self):
		...
		if self.random_initial_loc:
			# up, clockwise, radian, 15deg=math.pi/12
			self.agent_angle = (math.pi * random.randint(0, 23) / 12) % (2 * math.pi)
		else:
			self.agent_angle = (-math.pi / 12) % (2 * math.pi)
		...
		for i, ray in enumerate(self.lidar_rays):
			for j, shape in enumerate(self.object_bbox):
				...
				if min_dist < self.lidar_distance[i]:
					...
					if j == 1:
						self.detections[i] = [1, 0, 0]
		d1 = math.dist(self.agent_location, self.position1)
		d2 = math.dist(self.agent_location, self.position2)
		if self.dfa_state == '1':  # need to go to a -> award d1
			self.dist_prev = d1
		elif self.dfa_state == '3':  # need to go to b -> award d2
			self.dist_prev = d2
