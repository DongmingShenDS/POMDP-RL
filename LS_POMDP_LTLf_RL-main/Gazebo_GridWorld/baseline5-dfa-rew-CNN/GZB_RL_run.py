...
import os
from Gazebo_GW_env_cnn import GzbGw
...
def main():
	...
	ENV_COUNT = 1
	random_loc = True
	TIMESTEPS = 10240  # length of each episode
	SAVE_FREQ = 2
	...
	if algorithm == "PPO":
		# training env
		env = Monitor(GzbGw(
			"../../DFA/reachAavoidW.txt", update_img=True, monitor=have_monitor),
			logdir, info_keywords=("is_success", "final_DFA", "horizon", "total_reward")
		)
		if img_obs:
			model = PPO(
				"MultiInputPolicy", env, gamma=0.999, batch_size=256, learning_rate=2.5 * 1e-4,
