...
def main():
	...
	EVAL_FREQ = 50  # how many TIMESTEPS per evaluation
	TOTAL_SIM = 100
	...
	if algorithm == "PPO":
		# training env
		env = Monitor(GzbGw(
			"../../DFA/reachAavoidW.txt", img_obs=img_obs, update_img=have_monitor, monitor=have_monitor),
			logdir, info_keywords=("is_success", "final_DFA", "horizon", "total_reward")
		)
		# eval env
		eval_env = Monitor(GzbGw(
			"../../DFA/reachAavoidW.txt", img_obs=img_obs, update_img=have_monitor, monitor=have_monitor),
			logdir, info_keywords=("is_success", "final_DFA", "horizon", "total_reward")
		)
		if img_obs:
		...
		# training env
		env = GzbGw("../../DFA/reachAavoidW.txt", img_obs=img_obs, update_img=have_monitor, monitor=have_monitor)
		env = DummyVecEnv(
			[lambda: Monitor(env, logdir, info_keywords=("is_success", "final_DFA", "horizon", 'total_reward'))]
		)
		env = VecNormalize(env, norm_obs=True, norm_reward=True)
		# eval env
		eval_env = GzbGw("../../DFA/reachAavoidW.txt", img_obs=img_obs, update_img=have_monitor, monitor=have_monitor)
		...
		if img_obs:
			...
			model = DQN(
				...
				buffer_size=1000000, learning_starts=50000, batch_size=512, learning_rate=2.5 * 1e-4,
				exploration_final_eps=0.1, verbose=1, tensorboard_log=logdir, device='cuda:' + cuda_index
			)
	env.reset()
