...
class GzbGw(gym.Env):
	...
	def __init__(self, dfa, img_obs=False, actions=3, random_loc=True, monitor=False, update_img=False):
		"""
		...
		if self.if_image_obs:
			self.observation_space = spaces.Dict(
				spaces={
					"vec": spaces.Box(low=0, high=600, shape=(5,), dtype=np.float32),
	...
	def step(self, action):
		...
		if self.if_image_obs:
			self.observation = {
				"vec": np.array([ax, ay, theta, near_wall, dfa], dtype=np.float32),
				"img": self.img
			}
		else:
