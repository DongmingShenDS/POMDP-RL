...
class GzbGw(gym.Env):
	...
	def __init__(self, dfa, actions=3, random_loc=True, monitor=False, update_img=False, horizon=1500, rays=5):
		"""
		...
		"Observation Space"
		self.observation_space = spaces.Box(low=0, high=1200, shape=(self.RAY_COUNT * 4 + 4,), dtype=np.float32)
	...
	def step(self, action):
		...
		if self.dist_prev != -1:
			r = (self.dist_prev - dist_curr) / self.STEP_SIZE
		else:
			r = 0
		self.reward += r
		...
		others = np.array([ax, ay, theta, dfa], dtype=np.float32).flatten()
