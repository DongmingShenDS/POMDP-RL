...
class GzbGw(gym.Env):
	...
	def __init__(self, dfa, actions=3, random_loc=True, monitor=False, update_img=False, horizon=1024):
		"""
		...
		"Observation Space"
		self.observation_space = spaces.Box(low=0, high=1200, shape=(5,), dtype=np.float32)
	...
	def display_img(self):
		...
		"Display Objects"
		cv2.circle(self.img, self.position1, self.O_RADIUS, self.COLOR1, -1)
		cv2.circle(self.img, self.position2, self.O_RADIUS, self.COLOR2, -1)
		"Display Walls"
	...
	def step(self, action):
		...
		if original_dfa_state != new_dfa_state:
			print("DFA STATE changed from {} to {} at timestep {}".format(original_dfa_state, new_dfa_state, self.time_step))
		...
		for i, ray in enumerate(self.lidar_rays):
			for shape in self.wall_bbox:
				intersection, min_dist = ray_intersect_shape(ray, shape)
		...
		dfa = self.dfa_state
		self.observation = np.array([ax, ay, theta, near_wall, dfa], dtype=np.float32)
		"Info"
		info = {
...
def main():
	"""testing only"""
	line_intersection((0, 1), (1, 1), (0, 1), (1, 1))
	exit(0)
