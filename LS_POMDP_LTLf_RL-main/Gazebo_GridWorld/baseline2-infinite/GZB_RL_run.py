...
def main():
	...
	if algorithm == "PPO":
		# training env
		env = GzbGw("../../DFA/orderABavoidW.txt", update_img=have_monitor, monitor=have_monitor)
		env = DummyVecEnv(
			[lambda: Monitor(env, logdir, info_keywords=("is_success", "final_DFA", "horizon", 'total_reward'))]
		)
		env = VecNormalize(env, norm_obs=True, norm_reward=True)
		# eval env
		eval_env = GzbGw("../../DFA/orderABavoidW.txt", update_img=have_monitor, monitor=have_monitor)
		...
		if img_obs:
			...
			model = DQN(
				...
				buffer_size=1000000, learning_starts=50000, batch_size=512, learning_rate=1 * 1e-4,
				target_update_interval=50000, exploration_fraction=0.2, exploration_final_eps=0.05,
				verbose=1, tensorboard_log=logdir, device='cuda:' + cuda_index
			)
	env.reset()
		...
		if success_rate > VIDEO_EPS:
			...
			if algorithm == "PPO":
				...
				vid_env = Monitor(GzbGw(
					"../../DFA/orderABavoidW.txt", update_img=True, monitor=False),
				...
				# video env
				vid_env = GzbGw("../../DFA/orderABavoidW.txt", update_img=True, monitor=False)
				vid_env = DummyVecEnv(
